  ...
  <xsl:for-each select="
    ...
    <xsl:choose>
      <xsl:when test="@kind='class' or @kind='struct'">
        <xsl:if test="
            contains(compoundname, 'paxos::') and
...
<xsl:template name="strip-paxos-ns">
  <xsl:param name="name"/>
  <xsl:choose>
    <xsl:when test="contains($name, 'paxos::')">
      <xsl:value-of select="substring-after($name, 'paxos::')"/>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template match="ref[@kindref='compound']" mode="markup">
  ...
  <xsl:choose>
    <xsl:when test="contains(@refid, 'paxos') or contains($name, 'paxos::')">
      <xsl:variable name="dox-ref-id" select="@refid"/>
      <xsl:variable name="ref-name">
        <xsl:call-template name="strip-paxos-ns">
          <xsl:with-param name="name"
      ...
      <xsl:text>[link libpaxos_cpp.reference.</xsl:text>
...
<xsl:text>    </xsl:text><xsl:if test="@prot='public'">public </xsl:if><xsl:call-template
 name="strip-paxos-ns"><xsl:with-param name="name" select="."/></xsl:call-template><xsl:if
...
</xsl:if>``[link libpaxos_cpp.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of
...
['Inherited from <xsl:call-template name="strip-paxos-ns">
...
<xsl:text>
      ...
      <xsl:choose>
        ...
        <xsl:when test="declname = 'SocketService1' or declname = 'SocketService2'">
          <xsl:value-of select="concat('``[link libpaxos_cpp.reference.SocketService ', declname, ']``')"/>
        </xsl:when>
        ...
        <xsl:otherwise>
          <xsl:value-of select="concat('``[link libpaxos_cpp.reference.', declname, ' ', declname, ']``')"/>
        </xsl:otherwise>
      </xsl:choose>
...
<xsl:text>  </xsl:text><xsl:if test="string-length($stripped-type) &gt; 0"><xsl:value-of
 select="$stripped-type"/><xsl:text> </xsl:text></xsl:if>``[link libpaxos_cpp.reference.<xsl:value-of
